...
% Final results
method = {'SAUCE+MFR+Otzu'; 'SAUCE+MFR+ChanVese';  'SAUCE+MFR+Sobel'}; 
Accuracy=[m_O1(1);m_AC(1); m_SO(1)]; 
std_Accuracy=[std_O1(1);std_AC(1); std_SO(1)]; 

DSC=[m_O1(2);m_AC(2); m_SO(2)]; 
std_DSC=[std_O1(2);std_AC(2); std_SO(2)]; 

Sensitivity=[m_O1(3);m_AC(3); m_SO(3)]; 
std_Sensitivity= [std_O1(3);std_AC(3); std_SO(3)]; 

Specificity= [m_O1(4);m_AC(4); m_SO(4)];
std_Specificity= [std_O1(4);std_AC(4); std_SO(4)]; 

PPV= [m_O1(5);m_AC(5); m_SO(5)]; 
std_PPV= [std_O1(5);std_AC(5); std_SO(5)]; 
